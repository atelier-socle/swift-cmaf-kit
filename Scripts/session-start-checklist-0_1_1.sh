...
#
# session-start-checklist-0_1_1.sh
#
# Run at the START of every 0.1.1 implementation session.
# Confirms the branch is in a clean state and ready to receive changes.

set -euo pipefail
cd "$(dirname "$0")/.."

echo "→ Session start checklist (0.1.1 patch)"

# 1. On the right branch
BRANCH=$(git branch --show-current)
if [ "$BRANCH" != "feat/0_1_1-completion" ]; then
    echo "✗ Wrong branch: $BRANCH (expected feat/0_1_1-completion)"
    exit 1
fi
echo "✓ Branch: $BRANCH"
