...
#
# Generate per-target DocC archives for CMAFKit, CMAFKitDRM, and
# CMAFKitCLI, then merge them into a single
# ./docs/swift-cmaf-kit.doccarchive suitable for GitHub Pages.
set -euo pipefail
...
echo "→ Generating CMAFKitDRM.doccarchive"
swift package --allow-writing-to-directory ./docs \
    generate-documentation \
    --target CMAFKitDRM \
    --output-path ./docs/CMAFKitDRM.doccarchive
